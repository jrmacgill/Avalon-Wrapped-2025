...
const punConfig = {
  // Direct pun references (highest confidence)
  directMentions: ['pun', 'puns', 'punny', 'wordplay', 'dad joke', 'dad-joke'],

  // Wordplay patterns (homophones, malaprops, semantic shifts) - More precise
  wordplayPatterns: [
  ...
  // Contextual indicators (situational humor)
  contextPatterns: [
    /\b(like|i mean|but|so|well|yeah|anyway)\b.*\b(pun|puns|joke)\b/i,
    /\b(get it|you know|right|eh|aye)\b.*\?*$/i,
  ],

  // Emoji patterns indicating humor
  humorEmojis: ['😂', '🤣', '😆', '😅', '🙃', '😏', '🤪', '🤡', '🎪', '🎭', '🎨'],

  // Social validation (reactions are key indicator)
  reactionThreshold: 1,      // Any reaction shows engagement
  highReactionThreshold: 3,  // 3+ reactions = strong social validation

  // Linguistic characteristics
  shortMessageThreshold: 120,  // Puns are concise
  oneLinerThreshold: 50,       // Very short = high pun likelihood
}
...
function analyzePuns() {
  console.log(`🔍 Analyzing puns by user: ${targetUser}`)
  ...
  for (const file of jsonFiles) {
    ...
    messages.forEach(msg => {
      if (msg.isPun) {
        ...
        // Count indicators
        msg.indicators.forEach(indicator => {
          const key = indicator.split(':')[0] || indicator
          punIndicators[key] = (punIndicators[key] || 0) + 1
        })
      }
    })
  }
  ...
  console.log(`Estimated unique puns/bad jokes: ${punMessages.length}`)
  console.log(`Pun percentage: ${totalMessages > 0 ? ((punMessages.length / totalMessages) * 100).toFixed(1) : 0}%`)
  console.log()

  if (punMessages.length > 0) {
    console.log(`🎯 TOP PUNS (${Math.min(20, punMessages.length)} shown):`)
    console.log('-'.repeat(60))

    punMessages.slice(0, 20).forEach((msg, index) => {
      const date = new Date(msg.timestamp).toLocaleDateString()
      console.log(`${index + 1}. [${date}] #${msg.channel}`)
      console.log(`   "${msg.content}"`)
      console.log(`   Score: ${msg.score.toFixed(1)} | Reactions: ${msg.reactions}`)
      console.log(`   Indicators: ${msg.indicators.join(', ')}`)
      console.log()
    })

    console.log(`📈 PUN INDICATORS:`)
    Object.entries(punIndicators)
      .sort(([,a], [,b]) => b - a)
      .forEach(([indicator, count]) => {
        console.log(`   ${indicator}: ${count}`)
      })
  } else {
    console.log('No puns detected with current criteria.')
  }

  console.log()
  console.log('🎪 CONCLUSION:')
  if (punMessages.length >= totalMessages * 0.1) {
    console.log(`${targetUser} is definitely a pun enthusiast! ${punMessages.length} puns detected.`)
  } else if (punMessages.length >= totalMessages * 0.05) {
    console.log(`${targetUser} enjoys the occasional pun. ${punMessages.length} puns found.`)
  } else if (punMessages.length > 0) {
    console.log(`${targetUser} dabbles in puns occasionally. ${punMessages.length} pun-like messages.`)
  } else {
    console.log(`${targetUser} appears to be pun-free in 2025!`)
  }
}
